...
export function useSync(autoTrigger: boolean = true) {
  ...
  const pollSyncState = useCallback(async () => {
    ...
    try {
      ...
      // Supabase poll failed — keep polling
    }
  ...
  const triggerSync = useCallback(async () => {
    ...
    if (syncState.lastSyncAt) {
      ...
      if (Date.now() - lastSync < SYNC_CONFIG.debounceMinutes * 60 * 1000) {
        return;
      }
    }

    try {
      const res = await fetch(`${SYNC_CONFIG.triggerUrl}/trigger`, {
        method: "POST",
  ...
  useEffect(() => {
    async function loadInitialState() {
      ...
      try {
        ...
        // Failed to load initial state — not critical
      }
      initialLoadDone.current = true;
    }
    loadInitialState();
  ...
  useEffect(() => {
    if (!autoTrigger || !initialLoadDone.current) return;
    if (syncState.status === "running") return;

    const alreadyTriggered = sessionStorage.getItem(SESSION_KEY);
    if (alreadyTriggered) {
      const triggeredAt = parseInt(alreadyTriggered, 10);
      if (Date.now() - triggeredAt < SYNC_CONFIG.debounceMinutes * 60 * 1000) return;
    }

    if (syncState.lastSyncAt) {
      const lastSync = new Date(syncState.lastSyncAt).getTime();
      if (Date.now() - lastSync < SYNC_CONFIG.debounceMinutes * 60 * 1000) return;
    }
